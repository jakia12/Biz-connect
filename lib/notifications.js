...
export async function createOrderCancellationNotification(sellerId, orderId, orderNumber, reason) {
  try {
    await Notification.create({
      userId: sellerId,
      type: 'order_status',
      title: 'Order Cancelled',
      message: `Order #${orderNumber} was cancelled by the buyer. Reason: ${reason}`,
      link: `/dashboard/seller/orders/${orderId}`,
      metadata: { orderId, orderNumber, reason, status: 'cancelled' },
    });
  } catch (error) {
